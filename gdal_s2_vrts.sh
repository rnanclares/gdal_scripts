...
for imagen in *.zip; do
  ...
  unzip -Z -1 $imagen | grep -E B0._10m.jp2$ | sort | sed -e 's\^\/vsizip//'"$PWD/$(basename $imagen)"'/\' > bandList20m.txt
  unzip -Z -1 $imagen | grep -E B8A_10m.jp2$ | sort | sed -e 's\^\/vsizip//'"$PWD/$(basename $imagen)"'/\' >> bandList20m.txt
